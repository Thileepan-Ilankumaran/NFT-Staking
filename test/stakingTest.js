...
describe("Staking", () => {
  ...
  beforeEach(async () => {
    ...
    TOKEN = await ethers.getContractFactory("ThileepToken");
    token = await TOKEN.deploy(totalSupply);
    await token.deployed();

    // Deploy the ERC1155 NFT contract
    
    NFT = await ethers.getContractFactory("ThileepNFT");
    ...
    // transfer 1000 nfts to addr1,addr2
  ...
  describe("Minting nfts", function () {
    it("should assign the right amount of nfts to the owner of the contract", async function () {
      expect(await nft.balanceOf(owner.address, 0)).to.equal(1000);
      expect(await nft.balanceOf(owner.address, 1)).to.equal(1000);
      expect(await nft.balanceOf(owner.address, 2)).to.equal(1000);
    });
  });
  ...
  describe("staking", function () {
    it("should let users stake nfts", async () => {
      ...
      expect(await stakedNft.amount).to.equal(_amount);  // check if the amount was rightly set in mapping
